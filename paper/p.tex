...
Many workflows make use of similar analytical routines, such as data
filtering, FFT analysis, magnitude calculations of multi-dimensional data, etc.
However, one major limitation to existing workflow engines is that
...
common analytical procedures, for a scientist to assemble
and launch a workflow using existing tools,
significant effort is required to (a) allow this code
to operate on the specific data formats used in this
particular workflow, (b) map the workflow components
to the available resources, which in the HPC space
is usually a supercomputer,
and (c) reconcile the discrepancy between the parallelism
in one component and that of its downstream components.
...
A specific example illustrating the complexities comes from the Oak Ridge
...
%below may fit better in related work section.
