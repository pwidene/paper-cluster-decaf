...
\title{Turning Workflows Online: Tradeoffs and Tipping Points for Extreme Scale Workflows}

\author{
  \IEEEauthorblockN{Alexis Champsaur\IEEEauthorrefmark{1}, Jay Lofstead\IEEEauthorrefmark{2}, Jai Dayal\IEEEauthorrefmark{1}, Matthew Wolf\IEEEauthorrefmark{1}, Greg Eisenhauer\IEEEauthorrefmark{1}, Ada Gavrilovska\IEEEauthorrefmark{1}}
    \IEEEauthorrefmark{1}School of Computer Science, Georgia Institute of Technology}
  \IEEEauthorblockA{\IEEEauthorrefmark{2}Sandia National Laboratories
}
...
with rich functionality to manage the control flow. While these are quite functional, they have not been universally adopted due to complexity and limitations.
One example illustrating the complexities comes from the Oak Ridge
...
Proposed infrastructure for online workflows are more immature, but can offer
necessary
functionality~\cite{zheng:2010:predata,docan:2008:dataspaces,2007:wolf:smartpointer,2008:abbasi:datastager}.
None of these are fully sufficient for a robust infrastructure, but are being
extended to incorporate necessary management
functionality~\cite{2012:dayal:flexpath,2014:dayal:escience,2012:lofstead:txn,2012:lofstead:txn-pdsw,2013:lofstead:txn,2014:lofstead:txn,2016:lofstead:superglue}.
...
task, it is best to plan when to make the transition to ensure both sufficient
time and when the benefits outweigh the transition costs. New workflows can
...
% the below is covered in Design->Implementation Artifacts.
%ADIOS offers a
%simple I/O interface~\cite{lofstead:2009:adaptable}, which supports FlexPath
%as one of its underlying transport mechanisms. FlexPath supports data transport
%between distributed executables regardless of their placement.
