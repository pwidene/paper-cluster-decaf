...
interface provided by the ADIOS ~\cite{lofstead:2009:adaptable},
...
and (c) property indices. In this case, the per-timestep histogram
...
As we describe below, the granularity of these glue components is a bit
different than the coarse grained description made above. This is done to aid
re-usability, but achieve the same workflow with no custom glue code required.
At most, the user will specify a few parameters and organize the components
into a proper pipeline. Both of these operations are easy enough that a
non-expert application scientist can create workflows through GUIs or other
