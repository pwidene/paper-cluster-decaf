...
numbers, and (c) property indices. In this case, the per-timestep histogram
generated by the workflow will show the parallel pressure distributions across
...
configurable by the user at runtime is challenging.  In both workflows, the
...
calculation and the histogram generation, traditional glue code may be
reusable--but only for the pairing of these particular components.
...
In typical scientific simulation workflows today, custom glue code is written
for selecting relevant data and writing it for the magnitude calculator.
Another piece of custom glue code will adjust the data from the magnitude
calculation process for the histogram generator.  Potentially additional custom
glue code will fix the histogram output into something that can be rendered or
saved as desired.  In this work, we demonstrate general, reusable components
capable of handling the operation represented by all three intermediate
operations.

As we describe below, the granularity of these glue components is a bit
different than the coarse grained description above. This is done to aid
