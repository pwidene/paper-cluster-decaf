...
We demonstrated in the previous section the feasibility of
reusing SuperGlue components across workflows.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{|l|l|l|l|l|}
    \hline
    LAMMPS output size & AIO time (sec) & Superglue time (sec) \\
    \hline
    20 MB & 115.26 & 116.513\\
    \hline
    80 MB & 148.70 & 149.80\\
    \hline
    320 MB & 154.65 & 157.65\\
    \hline
    1280 MB & 155.32 & 157.98\\
    \hline
    5120 MB & 167.39 & 168.785\\
    \hline
  \end{tabular}
  %\vspace{-0.15in}
\end{table}
...
We measured the per-timestep throughput as well as
the start-to-end times of the two workflows 
at different scales.

~\autoref{tab:aio} shows the end-to-end
completion times of the entire workflow
at increasing scales. A weak scaling approach
is used, with approximately the same
per-process data size through the scaling.
The time is taken from the start of the simulation
...
approach involves more numerous data exchange points
...
until the readers are ready to receive it;
at this point, a separate thread in the writer
will handle the transfer.
This overlapping
of computation and I/O
amortizes the aforementioned negative effect.
...
SuperGlue workflows to allow the additional
components to run.
...
% I don't even mention the per-timestep
% throughput measurements, because
% they are difficult to talk about
