...
\includegraphics[width=\columnwidth]{fig/per-component-throughput}
\caption{GTCP Weak Scaling Experiment: Per-component, per-process throughput in KB/s.}
...
of the dataset sizes that they operate on and therefore
on the scale of their components.
Therefore, if they are to use generic components, it is important
...
The table lists the process sizes and dataset sizes used in the 5
...
timesteps broken down by component.

These measurements allow us to extract an
approximate per-process throughput, both for
the entire workflow, end-to-end, and on
a per-component basis.
The last table in ~\autoref{tab:weak-scaling-setup}
lists these end-to-end results, where the total
dataset sizes produced by the simulation are divided
...
shows per-component, per-process throughputs for
