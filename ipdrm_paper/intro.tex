...
due to their complexity and to the wide variety in data formats,
levels of parallelism, and arrangement of components that
they use. Still, many of these workflows use similar analytical
routines, albeit in different stages.

In this work, we presents \sys, a small set of
generic components that allow for the assembly
and launching of in situ workflows ``out of the box.''
That is, using the \sys approach, one can assemble entire workflows
from start to finish using a library of existing components,
without re-compilation of the components, and requiring only one-time
code modifications to the output methods of the driving simulation code.
